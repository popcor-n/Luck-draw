...
(function(){
    ...
    function tweenCircle(c, dir) {
        ...
        if(dir == 'in') {
            c.tween = TweenLite.to(c, 0.35, {x: c.originX, y: c.originY, ease:Quad.easeInOut, alpha: 1, radius: 5, scaleX: 0.4, scaleY: 0.4, onComplete: function() {
                c.movement = 'jiggle';
                tweenCircle(c);
            }});
        } else if(dir == 'out') {
            c.tween = TweenLite.to(c, 2.8, {x: window.innerWidth*Math.random(), y: window.innerHeight*Math.random(), ease:Quad.easeInOut, alpha: 0.2 + Math.random()*0.4, scaleX: 1, scaleY: 1, onComplete: function() {
                c.movement = 'float';
            ...
            if(c.movement == 'float') {
                c.tween = TweenLite.to(c, 10 + Math.random()*3.5, {x: c.x + -100+Math.random()*200, y: c.y + -100+Math.random()*200, ease:Quad.easeInOut, alpha: 0.2 + Math.random()*0.5,
                    onComplete: function() {
                        tweenCircle(c);
                    }});
             }// else {
...
function Choose(Names){
    ...
    if(Names[answer].name.length == 2){
        ...
        input = 'erro';
    }else if(Names[answer].name.length > 4){
        Names[answer].name = Names[answer].name.slice(0,3);
    }
    function cst(){
        str= Names[answer].sid;
    }
     cst();
     setTimeout('document.getElementById("xuehao").innerHTML = str',2800);
    
    return(Names[answer].name);
